...
- Composer install/update/require -> Mate first: `castor mate:composer-install`, `castor mate:composer-update --mode=summary`, `castor mate:composer-require vendor/package --constraint=^x.y` (schema-driven tasks in `mate:*`), or `mate/mate-tool-call.sh …` with JSON; fallback: `castor dev:composer-install` / `castor dev:composer "..."`
- PHPUnit / PHPStan -> Mate first: `castor mate:phpunit-run-suite --mode=summary`, `castor mate:phpstan-analyse --mode=summary` (or `mate/mate-tool-call.sh` with JSON); fallback: `castor dev:test` / `castor dev:phpstan`
...
- Required action: Load the `mate-tools` skill (`.cursor/skills/mate-tools/SKILL.md`) before Mate-related work; prefer `castor mate:*` or `mate/mate-tool-call.sh` per project hierarchy (Mate → Castor → raw commands).
